...
DF_24h_flat <- inner_join(DF_raw, DF_24h_flat, by = "ID") %>% filter(Date <= Sys.time())
...
DF_24h_summary <- 
    DF_24h_flat %>% group_by(Date) %>% summarise(sleep_24h = sum(value)) 
...
rbind(
    ...
    DF_24h_summary %>%
        ...
        mutate(subset = "last 7 days")) %>%
    filter(difftime(Date, min(Date), units = "days") >= 1) %>% 
    ggplot(aes(x = Date, y = sleep_24h, colour = subset)) + geom_point() + geom_smooth(method = "lm")
